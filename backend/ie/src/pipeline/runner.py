...
from .retrieval.runner import run_retrieval
...
class IERunner:
    ...
    @property
    def _retrieval_base(self) -> Path:
        return self._pipeline_dir / "outputs" / "retrieval"
    ...
    def _run_stage(self, stage: IEStage) -> None:
        dispatch = {
            ...
            IEStage.RETRIEVAL: self._run_retrieval,
            IEStage.FILTERING: self._run_filtering,
            IEStage.EXTRACTION: self._run_extraction,
        }
        dispatch[stage]()

    def _refresh_pmc_ids_csv(self) -> None:
        """Wget + gunzip NCBI's PMC-ids.csv unless already refreshed today.

        NCBI regenerates this file daily; re-downloading within the same day
        wastes bandwidth (~700 MB compressed) and time.
        """
        ncbi_dir = self._corpus_dir / "NCBI"
        csv_path = ncbi_dir / "PMC-ids.csv"
        if csv_path.exists():
    ...
    def _run_search(self) -> None:
        """Search PubMed/PMC and fetch missing BioC articles on demand.

        Work is staged at ``_YYYY_MM_DD`` and renamed to ``YYYY_MM_DD`` on
        success; an aborted run resumes from the staging dir next invocation.
        """
        s = self._s
        self._refresh_pmc_ids_csv()
    ...
    def _run_retrieval(self) -> None:
        """Extract food-chemical sentences from the local BioC cache."""
        s = self._s
        run_date = self._run_date()
        search_dir = self._search_base / run_date
        retrieval_dir = self._retrieval_base / run_date
        ...
        run_retrieval(
            query_uid_results_filepath=str(search_dir / "query_uid_results.tsv"),
            filtered_sentences_filepath=str(retrieval_dir / "result_{i}.tsv"),
            filepath_bioc_pmc=s.bioc_pmc_dir,
    ...
    def _run_filtering(self) -> None:
        ...
        if not retrieval_dir.exists():
            msg = f"Retrieval results not found: {retrieval_dir}"
            raise FileNotFoundError(msg)
        ...
        run_biobert_filter(
            input_file_path=str(retrieval_dir / "sentence_filtering_input.tsv"),
