...
from ..models.entity import ChemicalEntity, DiseaseEntity, FoodEntity
...
def create_chemicals_from_chebi(
    ...
    for _, row in nodes.iterrows():
        ...
        filtered_syns = [s.lower() for s in syns if s.lower() not in drop_names]
        entity = ChemicalEntity(
            foodatlas_id=f"e{store._curr_eid}",
            common_name=name,
            synonyms=filtered_syns,
            external_ids={"chebi": [int(row["native_id"])]},
        )
        rows.append(entity.model_dump(by_alias=True))
        for s in entity.synonyms:
            lut.add("chemical", s, entity.foodatlas_id)
        store._curr_eid += 1
