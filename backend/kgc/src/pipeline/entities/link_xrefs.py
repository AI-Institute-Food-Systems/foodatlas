...
These are cross-references (descriptors), not entity identifiers.
They are added to ``external_ids`` for data enrichment but are NOT
registered in the entity registry, because a single MeSH or PubChem
ID can legitimately map to multiple ChEBI entities.
"""
