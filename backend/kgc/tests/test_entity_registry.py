...
class TestResolve:
    ...
    def test_unknown_returns_empty(self, populated_registry: EntityRegistry) -> None:
...
class TestRegisterAlias:
    def test_new_alias(self, populated_registry: EntityRegistry) -> None:
        old = populated_registry.register_alias("cdno", "CDNO_1", "e2")
        assert old == ""
        assert populated_registry.resolve("cdno", "CDNO_1") == "e2"

    def test_same_entity_noop(self, populated_registry: EntityRegistry) -> None:
        old = populated_registry.register_alias("fdc", "100", "e1")
        assert old == ""

    def test_different_entity_returns_old(
        self, populated_registry: EntityRegistry
    ) -> None:
        old = populated_registry.register_alias("fdc", "100", "e2")
        assert old == "e1"
