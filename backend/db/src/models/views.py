...
class MVBioactivityEntity(Base):
    """Bioactivity concept pages: hierarchy + linked food/chemical counts."""
    ...
    external_ids: Mapped[dict] = mapped_column(JSONB, server_default="{}")
    parents: Mapped[list] = mapped_column(JSONB, server_default="[]")
    children: Mapped[list] = mapped_column(JSONB, server_default="[]")
    n_foods: Mapped[int] = mapped_column(Integer, server_default="0")
    n_chemicals: Mapped[int] = mapped_column(Integer, server_default="0")

    __table_args__ = (Index("ix_mv_bioent_common_name", "common_name"),)


class MVChemicalBioactivity(Base):
    """Denormalized chemical→bioactivity pairs with aggregated measurements.

    One row per (chemical, bioactivity) from r6 (MEASURED) triplets.
    """

    __tablename__ = "mv_chemical_bioactivity"
...
class MVFoodBioactivity(Base):
    """Denormalized food→bioactivity pairs with measurements.

    One row per (food, bioactivity) from r5 (EXHIBITS) triplets.
    """

    __tablename__ = "mv_food_bioactivity"
